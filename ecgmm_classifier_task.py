...
import torch
from ecgmm_incremental_task import CGMMTask
...
class ClassifierCGMMTask(CGMMTask):
    ...
    def run_valid(self, dataset_getter, logger):
        ...
        readout_class = s2c(config['readout'])
        model = readout_class(dim_node_features=dim_features,
                                dim_edge_features=0,
                                dim_target=dim_target,
                                config=config)

        readout_engine = self._create_engine(config, model, device, evaluate_every=self.model_config.evaluate_every)

        train_loss, train_score, _, \
        val_loss, val_score, _, \
        _, _, _ = readout_engine.train(train_loader=train_loader,
    ...
    def run_test(self, dataset_getter, logger):
        ...
        train_loss, train_score, _, \
        val_loss, val_score, _, \
        test_loss, test_score, _ = readout_engine.train(train_loader=train_loader,
                                                           validation_loader=val_loader,
