...
class TrainingEngine(EventDispatcher):
    ...
    def _to_data_list(self, data_list, x, batch, y):
        """
        Converts a graphs outputs back to a list of Tensors elements. Useful for incremental architectures.
        :param x: a tuple of embeddings: (vertex_output, edge_output, graph_output, other_output). Each of
        ...
        # count number of nodes per graph and accumulate
        ...
        # create one Data object for each graph in the batch
        new_data_list.append(Data(x=x[:cumulative[0]],
    ...
    def _to_list(self, data_list, embeddings, batch, edge_index, y):
        ...
        if data_list is None:
            data_list = []
        data_list = self._to_data_list(data_list, embeddings, batch, y)  # updates data list
        return data_list

    def _extract_num_targets(self, targets):
    ...
    def _loop(self, loader, return_node_embeddings=False):
        ...
        for _ in range(len(loader)):
            ...
            num_targets = self._extract_num_targets(targets)
...
class SingleGraphLinkPredictionEngine(TrainingEngine):
    """
    WARNING: _to_data_list (required for eventual subsequent training on
    node/graph embeddings) has to be extended to MINI-BATCH training, because
    it should return a list with a single Data object (for the entire graph)
    rather than many Data objects associated with different edge mini-batches
    """

    def _to_data_list(self, data_list, x, batch, y):

        # TODO here mini-batches are over edges. How can we get node embeddings
        # without repetitions?

        new_data_list = []
        # Return node embeddings and their original class, if any (or dumb value which is required nonetheless)
        y = y[0]
        new_data_list.append(Data(x=x, y=y[0]))
        data_list.extend(new_data_list)
