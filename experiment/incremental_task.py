...
class CGMMTask(Experiment):
    ...
    def __init__(self, model_configuration, exp_path):
        super(CGMMTask, self).__init__(model_configuration, exp_path)
    ...
    def run_valid(self, dataset_getter, logger):
        ...
        while not stop and depth <= max_layers:
            ...
            # TODO REFACTOR THIS (is specific to CGMM)
            #'''
            if stop:

                if 'CA' in self.model_config.layer_config:
                    # ECGMM
                    dim_features = new_layer.dim_node_features, new_layer.C * new_layer.depth + new_layer.CA * new_layer.depth if not new_layer.unibigram else (new_layer.C + new_layer.CA + new_layer.C * new_layer.C) * new_layer.depth
                else:
                    # CGMM
                    dim_features = new_layer.dim_node_features, new_layer.C * new_layer.depth if not new_layer.unibigram else (new_layer.C + new_layer.C * new_layer.C) * new_layer.depth
    ...
    def run_test(self, dataset_getter, logger):
        ...
        while not stop and depth <= max_layers:
            ...
            train_out = self._create_extra_dataset(prev_outputs_to_consider, mode='train', depth=depth, only_g_outs=True)
            val_out = self._create_extra_dataset(prev_outputs_to_consider, mode='validation', depth=depth, only_g_outs=True)
