...
from model.util.util import _compute_bigram, _compute_unigram, _make_one_hot
...
class CGMM(torch.nn.Module):
    ...
    def _compute_statistics(self, posteriors, data, device):
        ...
        statistics = torch.full((posteriors.shape[0], self.A + 1, posteriors.shape[1]+1), 0., dtype=torch.float32).to(device)
        srcs, dsts = data.edge_index

        if self.A == 1:
            sparse_adj_matr = torch.sparse_coo_tensor(data.edge_index, \
                                                      torch.ones(data.edge_index.shape[1], dtype=posteriors.dtype).to(device), \
...
class CGMMTransition(torch.nn.Module):
    def __init__(self, c, a, c2, l):
        ...
        self.layerS = torch.nn.Parameter(torch.nn.init.uniform_(torch.empty(self.L, dtype=torch.float32)), requires_grad=False)
        self.arcS = torch.nn.Parameter(torch.zeros((self.L, self.A), dtype=torch.float32), requires_grad=False)
        self.transition = torch.nn.Parameter(torch.empty([self.L, self.A, self.C, self.C2], dtype=torch.float32), requires_grad=False)
    ...
    '''
    NEVER USED. TODO Should also normalize the result if it computes a posterior.
